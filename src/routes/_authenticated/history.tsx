...
import { History as HistoryIcon, CheckCircle2, Clock, XCircle, AlertCircle, Search, X, RefreshCw, Landmark, Maximize2, Minimize2 } from "lucide-
react";
...
function PendingTimer({ startIso }: { startIso: string }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-amber-50 border border-amber-200 text-amber-700 text-[10px] font-
  ...
  return (
    ...
    >
      ...
      <div className="sticky top-0 z-20 bg-white/85 backdrop-blur-md pb-3">
        {/* Header card */}
        <div className="relative mt-1 mb-3 overflow-hidden rounded-2xl border border-black/5 bg-gradient-to-br from-[#0d4a3a] via-[#0f5a45] to-
[#127a5b] text-white shadow-sm">
          <div className="absolute inset-0 opacity-20 pointer-events-none" style={{ backgroundImage: "radial-gradient(circle at 20% 20%, white 0,
transparent 40%), radial-gradient(circle at 80% 80%, white 0, transparent 35%)" }} />
          <div className="relative px-3 sm:px-5 py-3 sm:py-4 flex flex-col gap-3">
            {/* Row 1: title + action buttons */}
            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
              <div className="w-10 h-10 sm:w-11 sm:h-11 shrink-0 rounded-xl bg-white/15 backdrop-blur-sm flex items-center justify-center ring-1
ring-white/20">
              ...
              <button
                ...
                className="shrink-0 inline-flex items-center gap-1.5 p-2 sm:px-3 sm:py-1.5 rounded-full bg-white/15 hover:bg-white/25 text-xs font-
semibold ring-1 ring-white/20 transition"
                aria-label="Refresh"
                title="Refresh"
              >
...
semibold ring-1 ring-white/20 transition"
            ...
            <div className="flex items-stretch gap-2 sm:gap-3">
               <div
                 className={`flex-1 min-w-0 flex flex-col px-3 py-2 rounded-xl bg-white/10 ring-1 transition-all ${pulse ? "ring-emerald-300/80
shadow-[0_0_20px_rgba(110,231,183,0.55)]" : "ring-white/20"}`}
        ...
        <div className="relative">
          ...
          <input
             ...
             className="w-full pl-9 pr-9 py-2.5 rounded-xl border border-black/10 bg-white text-sm shadow-sm focus:outline-none focus:ring-2
focus:ring-[#0d4a3a]/30 focus:border-[#0d4a3a]/40 transition"
          />
          {hasQuery && (
             <button
               onClick={() => setSearch("")}
...
[shimmer_1.4s_linear_infinite]"
          ...
          <div>
            ...
            <div className="hidden sm:grid sticky top-0 z-10 grid-cols-12 gap-3 px-5 py-2.5 bg-zinc-50/95 backdrop-blur text-[11px] uppercase
tracking-wider text-zinc-500 font-semibold border-b border-black/5">
