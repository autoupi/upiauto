...
pa=${encodeURIComponent(pa)}&pn=${encodeURIComponent(pn)}&am=${order.payable_amount.toFixed(2)}&cu=INR&tn=${order.order_id}`;
  ...
  if (!order) {
    return (
      <main className="fixed inset-0 h-[100dvh] w-[100dvw] overflow-hidden overscroll-none flex items-center justify-center p-3 sm:p-4" style={{
background: "#f5f5f5", touchAction: "none" }}>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-[0_2px_16px_-4px_rgba(0,0,0,0.08)] overflow-hidden">
            ...
            <div className="border-t border-gray-200 px-5 py-4 flex items-center justify-center">
              <div className="relative w-[185px] h-[185px] sm:w-[195px] sm:h-[195px] flex items-center justify-center">
                ...
                {showProcessing && (
                   <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                     <div className="flex items-center gap-1.5 bg-white/95 backdrop-blur-sm shadow-md rounded-full px-3 py-1.5 text-primary font-
semibold text-xs">
...
strokeLinejoin="round">
              ...
              <style>{`@keyframes tick-draw { to { stroke-dashoffset: 0; } } @keyframes copy-pop { 0% { transform: scale(0.6); opacity: 0; } 60% {
transform: scale(1.15); opacity: 1; } 100% { transform: scale(1); opacity: 1; } }`}</style>
            </div>
